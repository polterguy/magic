...
export class CrudifyComponent implements OnInit {
  ...
  tableChanged(e: MatSelectChange) {
    ...
    if (e.value === 'Custom SQL') {
      ...
      this.crudService.getColumns(this.databaseType, this.selectedDatabase, this.selectedTable).subscribe((res) => {
        this.columns = res;
        const generateChange = this.columns.filter(x => x.primary).length > 0;
        if (!generateChange) {
          this.showSuccess('No update and delete endpoints will be generated, since there is no primary key on your table');
        }
  ...
  disableGeneratorForVerb(el: any) {
    if (el.verb === 'delete' || el.verb === 'put') {
      if (!el.allowChange) {
        return true;
      }
    }
